...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int sum = 0;
        int salePrices[] = new int[products.length];
        boolean[] isOnSale = new boolean[products.length];

        for (int i = 0; i < products.length; i++) {
            if (counts[i] != 0) {
                salePrices[i] = prices[i] * counts[i];
                isOnSale[i] = false;
                ...
                if (isOnSale[i]) {
                    salePrices[i] = prices[i] * (counts[i] / 3 * 2 + counts[i] % 3);
                }
                sum += salePrices[i];
            }
        }
        ...
        for (int i = 0; i < products.length; i++) {
            if (counts[i] != 0) {
                System.out.println("\t" + products[i] + " " + (doBonus ? counts[i] + 1 : counts[i]) + " шт. за " + salePrices[i] + (isOnSale[i] ? " руб. (распродажа!)" : " руб."));
            }
        }
